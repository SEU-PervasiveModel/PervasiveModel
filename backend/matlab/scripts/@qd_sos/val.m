...
elseif any( size(cb) ~= size(ca) )
    error('qd_sos:val','Size of "cb" mut match the size of "ca".' );
end
...
if numel( h_sos ) > 1
    ...
    for i_sos = 1 : numel( h_sos )
        [ i1,i2,i3,i4 ] = qind2sub( sic, i_sos );
    ...
    if size( ca, 1 ) > dims
        error('qd_sos:val','Number of requested dimensions is not supported.' );
    end
    ...
    if ~isempty( cb )
        ...
        if all( abs( ph(:,1) - ph(:,2) ) < 1e-6 )
            ...
            A = mf.interp( h_sos.dist, 0, h_sos.acf, D );
