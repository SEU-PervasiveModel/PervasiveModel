...
zR = ones( dim, S, N, 'single' );

% Reconstructed ACF at test frequencies
ft = zeros( N, T, 'single' );
zT = ones( T, S, N, 'single' );           
...
while cnt > 0
    ...
    for n = 1 : N
        ...
        zRO = zR(:,:,n);            % Store approximate ACF
        ftO = ft(n,:);              % Store the test frequencies
        zTO = zT(:,:,n);            % Store test ACF
        ...
        if any( ls )
            RminuszR = R - sum( zR(dE,:,ls)/sum(ls) , 3 );
        else
            RminuszR = R;
        end
        
        % Calculate the best matching SOS frequency
        f = suosi_iteration_step( RminuszR, Dnorm, an );
