function [ xyzI, xyzR, r, lat, lonI, lonR, pq, Omega, omega, v ] = orbit_predictor( h_satellite, t, i_sat )
...
%
% Description:
%   Given the orbital elements given by the satellite' object, standard orbit mechanics are used
...
if numel( h_satellite ) > 1
    error('h_satellite:orbit_predictor','orbit_predictor not definded for object arrays.');
else
...
if ~exist('t','var')
    error('h_satellite:orbit_predictor','Input variable t is not defined.');
elseif ischar(t) && numel(t) == 6 && strcmp(t(1:3),'utc') && ~isempty( h_satellite.epoch )
    t = ( now - str2double(t(4:6))/24 - h_satellite.epoch)*86400;
elseif ~( isnumeric(t) && isreal(t) )
    error('h_satellite:orbit_predictor','Invalit time offset or epoch not given.');
else
    t = reshape( t,[],1 );
end
...
Omega = h_satellite.POmega_0(oT,i_sat) + t * h_satellite.Omega_r(1,i_sat);

% Perigee argument at timepoint t in [degrees]
omega = h_satellite.Pomega_0(oT,i_sat) + t * h_satellite.omega_r(1,i_sat);

% Mean anomaly at timepoint t in [degrees]
M = h_satellite.M_0(oT,i_sat) + t * h_satellite.n_bar(1,i_sat);
...
e = h_satellite.Pe( oT,i_sat );                      % The orbital eccentricity
...
inclination =  h_satellite.inclination( oT, i_sat );
xyzI = zeros( nT, nS, 3 );
if 0 % ITU proposal
    cO = cosd(Omega);
    sO = sind(Omega);
...
if isempty( h_satellite.epoch )  
    % Earths 0-meridian is aligned with vernal point at the simulation start
    GMST = h_satellite.Omega_e*t.';
else % Take Earths reference orientation into account
    dT = ( h_satellite.epoch - h_satellite.Epoch_eR ) * 86400; % Time difference in [s]
    GMST = h_satellite.Omega_e*t.' + h_satellite.Omega_e*dT + h_satellite.Omega_eR;
end
