function h_satellite = read_tle( filename )
...
semimajoraxis = ((period./(2*pi)).^2.*satellite.mu).^(1/3);
...
if any( upd )
    for n = find( upd )
        h_satellite = satellite('custom', semimajoraxis(n), eccentricity(n), inclination(n),...
            right_ascending_node(n), argument_of_perigee(n), true_anomaly(n));
        h_satellite.epoch = epoch(n);
        [ ~, ~, ~, ~, ~, ~, ~, right_ascending_node(n), argument_of_perigee(n), true_anomaly(n) ] =...
            h_satellite.orbit_predictor( (common_epoch - epoch(n))*86400 );
    end
end

% Create satellite constellation object
h_satellite = satellite('custom', semimajoraxis, eccentricity, inclination, right_ascending_node, argument_of_perigee, true_anomaly);
h_satellite.epoch = common_epoch;

ii = regexp(filename,'\.');
if numel( ii ) == 1
    h_satellite.name = filename(1:ii-1);
else
    h_satellite.name = filename;
end

% Set satellite name
for i_sat = 1 : h_satellite.n_satellites
    ...
    tmp = regexprep( tmp,' ','-' );
    h_satellite.sat_name{1,i_sat} = tmp;
end
