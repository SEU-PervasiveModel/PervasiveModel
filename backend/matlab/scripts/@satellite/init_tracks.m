function h_qd_track = init_tracks( h_satellite, ue_pos, t, i_sat, only_visible )
...
if numel( h_satellite ) > 1
    error('h_satellite:init_tracks','init_tracks not definded for object arrays.');
else
...
if only_visible
    [ ~,i_visible ] = h_satellite.ue_perspective( ue_pos, t+0.1 );   % Sat-positions and visibility
...
for n = 1 : nS
    ...
    [ xyzU,~,orientation ] = h_satellite.ue_perspective( ue_pos, t, iS ); 
    ...
    if ~isempty( h_satellite.sat_name )
        h_qd_track(1,n).name = h_satellite.sat_name{1,iS};
    end
