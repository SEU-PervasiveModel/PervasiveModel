function [ xyzU, visible, orientation ] = ue_perspective( h_satellite, ue_pos, t, i_sat )
...
if numel( h_satellite ) > 1
    error('h_satellite:ue_perspective','ue_perspective not definded for object arrays.');
else
...
elseif ~( isnumeric(t) && isreal(t) )
    error('h_satellite:init_layout','Invalit time offset or epoch not given.');
else
    t = reshape( t,[],1 );
end
...
xyzUE = satellite.R_e.*[  cosd(lonUE).*cosd(latUE); sind(lonUE).*cosd(latUE); sind(latUE) ];
...
[ xyzI, xyzR ] = orbit_predictor( h_satellite, t, i_sat );
...
if nargout > 2                                  % Get the orienation
    ...
    DxyzI = orbit_predictor( h_satellite, t+0.1, i_sat );
    D = DxyzI - xyzI;                          	% Speed vector in inertial frame
    
    % Calculate the z-rotation matrix for the Earths rotation
    GMST = h_satellite.Omega_e*t.';          % Angular of rotation of the Earth [deg]
    sLo = sind( GMST );
    cLo = cosd( GMST );
