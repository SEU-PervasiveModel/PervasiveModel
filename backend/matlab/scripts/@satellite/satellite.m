...
% Consructor:
%
%   h_satellite = satellite ( constellation, Ain, Bin, Cin, Din, Ein, Fin )
%   
...
methods
    function h_satellite = satellite( constellation, varargin )
        %SATELLITE Creates a new 'satellite' object (constructor)

        if nargin > 0 && ~isempty( constellation )
            h_satellite = satellite.generate( constellation, varargin{:} );
        end
    end
    
    % Get functions of public properties
    function out = get.n_satellites(h_satellite)
        out = numel( h_satellite.Pa );
    end
    function out = get.eccentricity(h_satellite)
        out = h_satellite.Pe;
    end
    function out = get.semimajor_axis(h_satellite)
        out = h_satellite.Pa;
    end
    function out = get.inclination(h_satellite)
        out = h_satellite.Pi;
    end
    function out = get.lon_asc_node(h_satellite)
        out = h_satellite.POmega_0;
    end
    function out = get.arg_periapsis(h_satellite)
        out = h_satellite.Pomega_0;
    end
    function out = get.true_anomaly(h_satellite)
        out = h_satellite.Pv_0;
    end
    
    % Get functions of dependent properties
    function out = get.R_p(h_satellite)          % Pericenter distance in [km]
        out = h_satellite.PR_p;
    end
    function out = get.R_a(h_satellite)          % Apocenter distance in [km]
        out = h_satellite.PR_a;
    end
    function out = get.p(h_satellite)            % Focal parameter in [km]
        out = h_satellite.Pp;
    end
    function out = get.n_0(h_satellite)          % Mean angular motion in [degrees/second]
        out = h_satellite.Pn_0;
    end
    function out = get.n_bar(h_satellite)        % Line of nodes in [degrees/second]
        out = h_satellite.Pn_bar;
    end
    function out = get.Omega_r(h_satellite)      % Ascending node longitude precession rate in [degrees/second]
        out = h_satellite.POmega_r;
    end
    function out = get.omega_r(h_satellite)      % Perigee argument precession rate in [degrees/second]
        out = h_satellite.Pomega_r;
    end
    function out = get.E_0(h_satellite)          % Initial eccentric anomaly in [degrees]
        out = h_satellite.PE_0;
    end
    function out = get.M_0(h_satellite)          % Initial mean anomaly in [degrees]
       out = h_satellite.PM_0;
    end
    function out = get.orbit_period(h_satellite) % Revised orbital period in [seconds]
        out = h_satellite.Porbit_period;
    end
        
    % Set functions
    function set.station_keeping(h_satellite, value)
        if numel(value) ~= 1
            error('satellite:wrongInputValue','??? "station_keeping" must be a scalar');
        end
        h_satellite.station_keeping = logical( value );
        update_dependent_variables( h_satellite );
    end
    
    function set.semimajor_axis(h_satellite, value)
        if ~(isnumeric(value) && isreal(value) && (size(value, 1) == 1) && all(value > 6378.145))
            error('satellite:wrongInputValue',...
                '??? "semimajor_axis" must be a real, numeric, row vector, having values greater than 6378.145 km');
        end
        no = numel( h_satellite.Pa );            % Number of exisiting satellites
        ii = 1:min(no,numel( value ));              % Indices of exisiting satellites
        init = zeros( 1,numel(value) );             % Initial values for all other variables
        h_satellite.Pa = value;                  % Set new value
        if no > 0; init( 1,ii ) = h_satellite.Pe( 1,ii ); end
        h_satellite.Pe = init;                   % Update dependent property "Pe"
        if no > 0; init( 1,ii ) = h_satellite.Pi( 1,ii ); end
        h_satellite.Pi = init;                   % Update dependent property "Pi"
        if no > 0; init( 1,ii ) = h_satellite.POmega_0( 1,ii ); end
        h_satellite.POmega_0 = init;             % Update dependent property "POmega_0"
        if no > 0; init( 1,ii ) = h_satellite.Pomega_0( 1,ii ); end
        h_satellite.Pomega_0 = init;             % Update dependent property "Pomega_0"
        if no > 0; init( 1,ii ) = h_satellite.Pv_0( 1,ii ); end
        h_satellite.Pv_0 = init;                 % Update dependent property "Pv_0"
        update_dependent_variables( h_satellite );
    end
    
    function set.eccentricity(h_satellite, value)
        ...
        if numel( value ) ~= numel( h_satellite.Pa )
            error('satellite:wrongInputValue',...
                '??? number of elements must match the number of satellites. Set "semimajor_axis" first.')
        end
        h_satellite.Pe = value;                  % Set new value
        update_dependent_variables( h_satellite );
    end
    
    function set.inclination(h_satellite, value)
        if ~(isnumeric(value) && isreal(value) && (size(value, 1) == 1) && all(-90 <= value & value <= 180))
                error('qd_constellation:wrongInputValue',...
        ...
        h_satellite.Pi = value;                  % Set new value
        update_dependent_variables( h_satellite );
    end
    
    function set.lon_asc_node(h_satellite, value)
        ...
        value( value == -180 ) = 180;
        h_satellite.POmega_0 = value;        	% Set new value
        update_dependent_variables( h_satellite );
    end
    
    function set.arg_periapsis(h_satellite, value)
        if ~(isnumeric(value) && isreal(value) && (size(value, 1) == 1))
            error('qd_constellation:wrongInputValue',...
    ...
    function set.true_anomaly(h_satellite, value)
        if ~(isnumeric(value) && isreal(value) && (size(value, 1) == 1))
            error('qd_constellation:wrongInputValue',...
        ...
        value = mod( value +180,360)-180;           % Map to (-180,180]
        value( value == -180 ) = 180;
        h_satellite.Pv_0 = value;                % Set new value
        update_dependent_variables( h_satellite );
    end
    
    function update_dependent_variables( h_satellite )
        % Pericenter distance in [km]
        h_satellite.PR_p = (1-h_satellite.Pe).*h_satellite.Pa;
        
        % Apocenter distance in [km]
        h_satellite.PR_a = (1+h_satellite.Pe).*h_satellite.Pa;
        
        % Focal parameter in [km]
        h_satellite.Pp = h_satellite.Pa.*(1 - h_satellite.Pe.^2);
        
        % Mean angular motion in [degrees/second]
        h_satellite.Pn_0 = sqrt(h_satellite.mu./h_satellite.Pa.^3)/pi*180;
        
        % Line of nodes in [degrees/second]
        if h_satellite.station_keeping
            h_satellite.Pn_bar  = h_satellite.Pn_0;
        else
            h_satellite.Pn_bar  = h_satellite.Pn_0 .* (1 + 1.5*h_satellite.J_2 .* ...
                h_satellite.R_e.^2 ./ h_satellite.Pp.^2.* ...
                (1 - 1.5*sind(h_satellite.Pi).^2).*sqrt(1-h_satellite.Pe.^2));
        end
        
        % Ascending node longitude precession rate in [degrees/second]
        if h_satellite.station_keeping
            h_satellite.POmega_r = zeros( size( h_satellite.Pa ) );
        else
            h_satellite.POmega_r = -1.5 * h_satellite.J_2 .* h_satellite.R_e.^2 ./ ...
                h_satellite.Pp.^2 .* h_satellite.Pn_bar .* cosd(h_satellite.Pi);
        end
        ...
        if h_satellite.station_keeping
            ...
            h_satellite.Pomega_r = 1.5*h_satellite.J_2 .* h_satellite.R_e.^2 ./ ...
                h_satellite.Pp.^2.* h_satellite.Pn_bar.*(2-2.5.*sind(h_satellite.Pi).^2);
        end
            
        % Initial eccentric anomaly in [degrees]
        Pex = h_satellite.Pe;
        h_satellite.PE_0 = 2*atand(tand(h_satellite.Pv_0./2).*sqrt((1-Pex)./(1+Pex)));
        
        % Initial mean anomaly in [degrees]
        h_satellite.PM_0 = h_satellite.PE_0 - h_satellite.Pe.*sind(h_satellite.PE_0);
        
        % Revised orbital period in [seconds]
        h_satellite.Porbit_period = 360./(h_satellite.Pomega_r + h_satellite.Pn_bar);
    end
end

methods(Static)
    h_satellite = generate( constellation, Ain, Bin, Cin, Din, Ein, Fin );
    h_satellite = read_tle( filename );
end
end
